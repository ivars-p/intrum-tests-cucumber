package com.intrum.tests.pages.contactUsPage;
...
import static com.intrum.tests.constants.contactUs.commentAndcomplaintForm.FormsFieldNotifications.EMPTY_FIELD_NOTIFICATION;
import static com.intrum.tests.constants.contactUs.commentAndcomplaintForm.FormsTitles.COMMENTS_ADN_COMPLAINTS_FORM_TITLE;
import static com.intrum.tests.constants.contactUs.commentAndcomplaintForm.FormsTitles.SUCCESSFULLY_SUBMITTED_FORM;
import static com.intrum.tests.helpers.TestCaseContext.getUser;
...
public class CommentsAndComplaintsForm extends BasePage {
    ...
    protected CommentsAndComplaintsForm validateFormsTitle(By element, String expectedTitle) {
        String title = wait.forElementToBeDisplayed(element, "Forms title: " + expectedTitle).getText();
    ...
    public CommentsAndComplaintsForm fillCommentsAndComplaintsForm() {
        fillInputField(NAME_AND_SURNAME_LCTR, getUser().getName() + ", " + getUser().getSurname());
        fillInputField(PERSONAL_CODE_LCTR, getUser().getPersonalCode());
        fillInputField(CASE_NUMBER_LCTR, getUser().getCaseNumber());
        fillInputField(CONTACT_NUMBER_LCTR, getUser().getPhone());
        fillInputField(EMAIL_LCTR, getUser().getEmail());
        fillInputField(ADDRESS_LCTR, getUser().getAddress());
        fillTextAreaField(COMMENT_AND_COMPLAINT_LCTR, getUser().getComplaint());
        selectValueFromDropDown(RESPONSE_TYPE_LCTR, getUser().getResponseType().toString());
        return this;
    }
    ...
    public CommentsAndComplaintsForm verifyEmptyFieldNotificationHard(By element) {
        String notification = wait.forElementToBeDisplayed(new ByChained(element, ERROR_NOTIFICATION_LCTR), "Empty field notification").getText();
